...
int main(int argc, char*argv[])
{
	...
	char * rtp_pkt;
	uint8_t default_aes_keys[AES_KEY_SIZE], default_hmac_keys[HMAC_KEY_SIZE];
	...
	//printf("Num of flows is %d, pkt per blk is %d,  threads_per_blk is %d, num of blocks is %d, pkt size is %d, stream num is %d\n", 
	//	NUM_FLOWS, NUM_FLOWS/NUM_BLKS, THREADS_PER_BLK, NUM_BLKS, PKT_SIZE, STREAM_NUM);
	//printf("###############################################\n");
	...
	fsize = PKT_SIZE;
	pad_size = (((PKT_SIZE + 3) & ~0x03) + 63 + HMAC_TAG_SIZE) & (~0x03f);
