...
static IP_THREAD_LOCAL_P ip_thread_local_struct;
static TCP_THREAD_LOCAL_P tcp_thread_local_struct;
...
//	printf("HEY, IAM AP : %d\n", thread_id);
	...
	while (1) { /* loop "forever" */
		...
		gen_ip_frag_proc(elem.data, elem.len, thread_id);
	}
end_loop:
	gettimeofday(&t2, 0);
...
int nids_lib_init(int n_cores, _IP_THREAD_LOCAL_P *ip_thread_local_struct_in, _TCP_THREAD_LOCAL_P *tcp_thread_local_struct_in) {
	assert(n_cores < MAX_CPU_CORES);
	init_procs();
	ip_thread_local_struct = ip_thread_local_struct_in;
	tcp_thread_local_struct = tcp_thread_local_struct_in;
	int i;
	for (i = 0; i < n_cores; i++) {
		...
		tcp_thread_local_struct[i].self_cpu_id = i; // TODO reverse
		tcp_init(nids_params.n_tcp_streams, &tcp_thread_local_struct[i]);
